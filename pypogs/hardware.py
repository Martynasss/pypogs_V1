...
class Mirror:
    def __init__(self, model=None, identity=None):
        """Create Mount instance. See class documentation."""
        # Start of constructor
        self._model = None
        self._name = 'MEMS'
        ...
        self._bias_value = ['M3026010', 'M3125615', 'M3225620', 'M3325722']
        self._prior_values = [26010, 25615, 25620, 25722]
        self._max_step = 1730  # 4V step
        self.initialize()
    ...
    def send_X(self, value):
        ...
        value_X_dec = int(value_KX*value_X_voltage*327.675)
        ...
        if (self._minLimit < value_X_dec < self._maxLimit and
            ...
            # Check step size, if too big only Limited amount will be added
            if (abs(self._prior_values[0] - value_X_dec) > self._max_step and
                    abs(self._prior_values[1] - value_nX_dec) > self._max_step):
                # STEP size too big, only limited step will be added
                if value_X_dec > self._prior_values[0]:
                    value_X_dec = self._prior_values[0] + self._max_step
                else:
                    value_X_dec = self._prior_values[0] - self._max_step
                if value_nX_dec > self._prior_values[1]:
                    value_nX_dec = self._prior_values[1] + self._max_step
                else:
                    value_nX_dec = self._prior_values[1] - self._max_step
                print('ERROR: Step size to BIG ch_X')

            self._prior_values[0] = value_X_dec
            self._prior_values[1] = value_nX_dec
            # SEND VOLTAGE TO MIRROR
            ...
            #Voltage exceed one of the limits
            print('ERROR: Voltage limit exceeded ch_X')
    ...
    def send_Y(self, value):
        ...
        if (self._minLimit < value_Y_dec < self._maxLimit and
            ...
            # Check step size, if too big only Limited amount will be added
            if (abs(self._prior_values[2] - value_Y_dec) > self._max_step and
                    abs(self._prior_values[3] - value_nY_dec) > self._max_step):
                # STEP size too big, only limited step will be added
                if value_Y_dec > self._prior_values[2]:
                    value_Y_dec = self._prior_values[2] + self._max_step
                else:
                    value_Y_dec = self._prior_values[2] - self._max_step
                if value_nY_dec > self._prior_values[3]:
                    value_nY_dec = self._prior_values[3] + self._max_step
                else:
                    value_nY_dec = self._prior_values[3] - self._max_step
                print('ERROR: Step size to BIG ch_Y')

            self._prior_values[2] = value_Y_dec
            self._prior_values[3] = value_nY_dec
            # SEND VOLTAGE TO MIRROR
            # send Y positive channel
            value_Y_dec = 'M32' + str(value_Y_dec)
            self.send_command(value_Y_dec)
            # send Y minus channel
            value_nY_dec = 'M33' + str(value_nY_dec)
            self.send_command(value_nY_dec)
        else:
            print('ERROR: Voltage limit exceeded ch_Y')
