...
class TrackingThread:
    ...
    def _run(self):
        ...
        loop_index = 0
        # print("mygtukas RUN paspaustas")
        # print("starto laikas", start_timestamp)
        # print("senas laiaks", old_timestamp)
        try:
            while not self._stop_running:
                ...
                image = self._image_data.copy() # cia kiekviena karta gauna nauja nuotrauka sudaryta is dec reiksmes viename pixelyje
                loop_timestamp = precision_timestamp() - start_timestamp
                # print("loop laikas", loop_timestamp, precision_timestamp(),start_timestamp)
                ...
                # print("dt",dt,"loop laikas", loop_timestamp,"minus senas laiaks", old_timestamp )
    ...
    def track_alt_az(self):
        ...
        if not self.is_running:
            return (None, None)
        st_track = np.array(self._spot_tracker.track_x_y) # paklaida nuo centro (arsec), padaro i matrica t.y is () i [] ir sukeicia vietomis

        if None in st_track:
            return (None, None)
        else:
            rotation = self._camera.rotation # cia yra nurodyta rotacija dabar 0.0 nieko nedaro
            rotmx = np.array([[np.cos(np.deg2rad(rotation)), np.sin(np.deg2rad(rotation))],  # cia konvertuota ir persukta matrica, nesupratau
                              [-np.sin(np.deg2rad(rotation)), np.cos(np.deg2rad(rotation))]])
            return tuple(rotmx @ st_track)[::-1] # po visu siu proceduru atiduoda sukeista vietomis x su y  kas buvo track_x_y (dabar sukeista vietomis reiksmes, kodel??? kam to reikia
...
class SpotTracker:
    ...
    You may select to use your own spot detection system and directly update the tracker with
    ...
    def update_from_image(self, img, plate_scale=1):
        ...
        if self.crop is not None:
            ...
            if len(self.crop) == 2:
                ...
                cr = list(self.crop[1::-1]) + list(self.crop[3:1:-1]) # cia atiduoda reiksmes kaip lista, t.y. naujos nuotraukos dydis x y ir centro taskai 0.0 ir 0.0
        else:
            cr = None
        ...
        if self._has_track:
            ...
            if ret[0][:, 0].size > 0:
                ...
                if None not in (x_search, y_search):
                    ...
                    dx = np.abs(x - x_mean)
                    print("NERA, search x:", dx)
                    dy = np.abs(y - y_mean)
                index = (dx ** 2 + dy ** 2).argmin()
                print("indekso reiksme", index)
                x = float(x[index])
                y = float(y[index])
                dx = dx[index]
                dy = dy[index]
                if search_rad is None or (abs(dx) < search_rad and abs(dy) < search_rad):
                    ...
                    success = True
            if not success:
                print('neturi centro matricos sekme tampa 0')
                self._success_count = 0
                self._fail_count += 1
                print("+1nesekme")
                print((self._fail_count))
                print('kreipesi i abservation def ir nustato none pas ji')
            ...
            if ret[0][:, 0].size > 0:
                self._success_count += 1
                self._fail_count = 0
                x = (ret[0][:, 1] - imshape[1] / 2) * plate_scale # cia jis apskaiciuoja paklaida px nuo foto centro ir centroido centro padaugina is plate scale ir gaunama paklaida x asyje arcsec
                # Image Y coordinates increase down!
                y = - (ret[0][:, 0] - imshape[0] / 2) * plate_scale # tas pats kas su x
                (x_search, y_search) = self.pos_search_x_y
                if self._auto_aquire:
                    ...
                    y = float(y[0])
                    self.update_from_observation(x, y, summ, area)
                    print("kreipesi i abservation def")
                    if self._success_count >= self._succ_to_start:
                        self._log_info('Has track''Found track 3 succes in row')
                ...
                self.update_from_observation(None, None, None, None)
                self.clear_tracker()
        end_timestamp = precision_timestamp()
    ...
    def update_from_observation(self, x, y, summ, area):
        ...
        if None in (x, y, summ, area):
            ...
            if None in (self._x_mean, self._y_mean, self._pos_variance, self._sum_mean,
                ...
                dx = x - self._x_mean # cia apskaiciuoja taip: praeito uzejimo x paklaida nuocentro atimti dar praeito ir gauname skirtuma tarp praeito matavimo ir sio matavimo
                dy = y - self._y_mean
                ...
                if self._has_track:
                    self._pos_search_x = self._x_mean # cia prilygina ieskoma regiona su esamu vidurkiu
...
class MemsThread:
    def __init__(self, parent, name=None, camera=None):

        from . import system
        assert isinstance(parent, system.System), 'Parent must be pypogs.System instance.'

        # Start of constructor
        self._data_folder = Path(__file__).parent / 'data'  # Data folder setup
        self._model = None
        self._stop_running = True
        self._thread = None
        # Callbacks on image event
        ...
        self._fb_ki = 1
        self._x_diff_mV = 0
        self.diff_mV = np.array([0, 0], dtype='float64')
        self.integral_limit = 100000  #integral_limit * _fb_ki = mV
        self._maxLIM = 70000 # 90V
        self._minLIM = -70000 # -90V
    ...
    def start(self):
        """Starts the tracking in a background thread. Must have Camera and SpotTracker."""
        self._stop_running = False
        self._thread = Thread(target=self._run, name=self._name + 'Worker')
        self._thread.start()
        self._parent.mirror.start()
    ...
    def _run(self):
        ...
        # Create logfile
        start_time = apy_time.now()
        data_filename = Path(start_time.strftime('%Y-%m-%dT%H%M%S') + '_MEMSLoopThread.csv')
        data_file = self.data_folder / data_filename
        if data_file.exists():
            self._log_debug('File name clash. Iterating...')
            append = 1
        ...
        with open(data_file, 'w') as file:
            ...
            writer.writerow(['T_UTC_ISO', 'dt_ms', 'ErrorX(pix)', 'ErrorY(pix)','Kp', 'Ki', 'P_term_x', 'I_term_x',
                             'P_term_y', 'I_term_y', 'Diff_mV X', 'Diff_mV Y','Goal X','Goal Y'])


        try:
            while not self._stop_running:
                # _________________Synchronisation and time management______________________
                if not self._process_image.wait(timeout=3):
                    print('Timeout waiting for image MEMS')
                loop_utctime = apy_time.now()  # Astropy timestamp in UTC
                loop_timestamp = precision_timestamp() - start_timestamp  # reikia keleta kartu atspausdinti kad issiaiskinti kas cia kaip kam
                dt = loop_timestamp - old_timestamp
                old_timestamp = loop_timestamp
                ...
                if not np.isnan(err_alt_az_pix[0]):   # jei nera tracko tai prasoka sita dali
                    ...
                    if abs(err_integral[0]) < self.integral_limit:
                        err_integral[0] = err_integral[0] + err_alt_az_pix[0] / dt  # stai cia pradeda sumuotis musu klaidos x reiksmei nesuprantu kodel reikia padialinti is dt
                ...
                with open(data_file, 'a') as file:
                    ...
                    writer.writerow([loop_utctime.isot, (dt * 1000), err_alt_az_pix[0], err_alt_az_pix[1], self._fb_kp, self._fb_ki,
                                     err_alt_az_pix[0], err_integral[0],
                                     err_alt_az_pix[1], err_integral[1],
                                     self.diff_mV[0], self.diff_mV[0] ]) #,self._goal_x_y[0], self._goal_x_y[1]
    ...
    def limit_output(self):
        if self.diff_mV[0] > self._maxLIM:
            self.diff_mV[0] = self._maxLIM
        if self.diff_mV[0] < self._minLIM:
            self.diff_mV[0] = self._minLIM

        if self.diff_mV[1] > self._maxLIM:
            self.diff_mV[1] = self._maxLIM
        if self.diff_mV[1] < self._minLIM:
            self.diff_mV[1] = self._minLIM


    def stop(self):
        """Stop the thread."""
        self._stop_running = True
        self._parent.mirror.stop()
