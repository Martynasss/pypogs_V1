...
from .hardware import Camera, Mount, Receiver, Mirror
from .tracking import TrackingThread, ControlLoopThread, MemsThread
...
class System:
    ...
    def deinitialize(self):
        ...
        if self.mirror is not None:
            self._logger.debug('Has mirror')
            if self.mirror.is_init:
                try:
                    self.mirror.deinitialize()
                    self._logger.debug('Deinitialised')
                except BaseException:
                    self._logger.warning('Failed to deinit', exc_info=True)
            else:
    ...
    @property
    def mirror_thread(self):
        """System.ControlLoopThread: Get the system control loop thread."""
        return self._mirror_thread
    ...
    @property
    def mirror(self):
        """pypogs.Mirror: Get or set the mirror."""
        return self._mirror
    ...
    def add_mirror(self, *args, **kwargs):
        """Create and set a pypogs.Mirror for System.mount. Arguments passed to constructor.
        Args:
        """
        if self.mirror is not None:
            self.mirror = None
